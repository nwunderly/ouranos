...
class General(Cog):
    ...
    async def about(self, ctx):
        ...
        owner = await self.bot.get_or_fetch_member(
            self.bot.get_guild(Settings.guild_id), Settings.owner_id
        )
    ...
    async def invite(self, ctx):
        ...
        if await is_bot_admin(ctx.author):
            ...
            await ctx.send(
                f"This bot is currently private. Please contact {Settings.author} if interested in using it."
            )
    ...
    async def ping(self, ctx):
        ...
        append = (
            ...
            else (PINGBOI if self.bot.user in ctx.message.mentions else "")
        )
    ...
    async def _cleanup(self, ctx, limit: int = 30):
        ...
        p = tuple(
            [
                ...
                f"<@!{self.bot.user.id}>",
                await self.bot.prefix(ctx.message),
            ]
        )
